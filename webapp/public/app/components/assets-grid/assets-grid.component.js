"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var router_1 = require('@angular/router');
var asset_service_1 = require('../../services/asset.service');
var AssetsGridComponent = (function () {
    function AssetsGridComponent(_assetService, _router) {
        this._assetService = _assetService;
        this._router = _router;
        this.assets = [];
    }
    AssetsGridComponent.prototype.ngOnInit = function () {
        var _this = this;
        this._assetService.getAssets()
            .then(function (assets) { return _this.assets = assets; });
    };
    AssetsGridComponent.prototype.new = function () {
        this._router.navigate(['/asset']);
    };
    AssetsGridComponent.prototype.delete = function (id) {
        var _this = this;
        this._assetService.delete(id)
            .then(function () { return _this._assetService.getAssets()
            .then(function (assets) { return _this.assets = assets; }); });
    };
    AssetsGridComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
            selector: 'assets-grid',
            templateUrl: 'assets-grid.component.html'
        }), 
        __metadata('design:paramtypes', [asset_service_1.AssetService, router_1.Router])
    ], AssetsGridComponent);
    return AssetsGridComponent;
}());
exports.AssetsGridComponent = AssetsGridComponent;
//# sourceMappingURL=assets-grid.component.js.map